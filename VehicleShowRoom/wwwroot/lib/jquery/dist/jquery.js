...
// ReSharper disable InvokedExpressionMaybeNonFunction
...
// ReSharper disable AssignmentInConditionExpression
 ...
 */
				function siblingCheck(a, b) {
					var cur = b && a;
					const diff = cur && a.nodeType === 1 && b.nodeType === 1 && (~b.sourceIndex || MAX_NEGATIVE) - (~a.sourceIndex || MAX_NEGATIVE);
 ...
 */
				setDocument = Sizzle.setDocument = function(node) {
					...
					if ((support.matchesSelector =
						rnative.test((matches = docElem.matches || docElem.webkitMatchesSelector || docElem.mozMatchesSelector || docElem.oMatchesSelector || docElem.msMatchesSelector)))) {
				...
				function setMatcher(preFilter, selector, matcher, postFilter, postFinder, postSelector) {
					...
					return markFunction(
						function(seed, results, context, xml) {
							...
							const matcherIn = preFilter && (seed || !selector) ? condense(elems, preMap, preFilter, context, xml) : elems;
		...
		jQuery.fn.extend(
			{
				...
				closest: function(selectors, context) {
					...
					const l = this.length;
					const matched = [];
					const pos = rneedsContext.test(selectors) || typeof selectors !== 'string' ? jQuery(selectors, context || this.context) : 0;
...
// ReSharper disable once Html.Obsolete
		...
		var swap = function(elem, options, callback, args) {
			...
			var ret = callback.apply(elem, args || []);
			...
			jQuery.extend(
				support,
				{
					...
					reliableMarginRight: function() {
						...
						var ret = !parseFloat(window.getComputedStyle(marginDiv).marginRight);
		...
		Tween.propHooks = {
			_default: {
				get: function(tween) {
					...
					var result = jQuery.css(tween.elem, tween.prop, '');
