...
// ReSharper disable InconsistentNaming
				var firstCN = parseInt(value.substring(9, 10), 10);
				var secondCN = parseInt(value.substring(10, 11), 10);
 ...
 */
		$.validator.addMethod(
			...
			function(phoneNumber, element) {
				...
				return this.optional(element) || phoneNumber.length > 9 && phoneNumber.match(/^(?:(?:(?:00\s?|\+)44\s?|0)7(?:[1345789]\d{2}|624)\s?\d{3}\s?\d{3})$/);
			},
			'Please specify a valid mobile number');
...
//Matches UK landline + mobile, accepting only 01-3 for landline or 07 for mobile to exclude many premium numbers
		$.validator.addMethod(
			...
			function(phoneNumber, element) {
				phoneNumber = phoneNumber.replace(/\(|\)|\s+|-/g, '');
				return this.optional(element) || phoneNumber.length > 9 && phoneNumber.match(/^(?:(?:(?:00\s?|\+)44\s?|0)(?:1\d{8,9}|[23]\d{9}|7(?:[1345789]\d{8}|624\d{6})))$/);
			},
 ...
 */
		$.validator.addMethod(
			...
			function(value, element, options) {
				...
				const numberFilled = $fields.filter(
						function() {
							return validator.elementValue(this);
						})
					.length;
